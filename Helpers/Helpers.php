<?php 
    ...
    function sendEmail($data,$template)
    {
        if(ENVIRONMENT == 1){
            ...
            try {
                ...
                $mail->Username   = 'rjcm.caira@gmail.com';                     //SMTP username
                $mail->Password   = 'sqxkcbflwzosudbe';                               //SMTP password
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
                $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

                //Recipients
                $mail->setFrom('rjcm.caira@gmail.com', 'Servidor Local');
    ...
    function sendMailLocal($data,$template){
        ...
        try {
            ...
            $mail->Host       = 'smtp.gmail.com';                     //Set the SMTP server to send through
            $mail->SMTPAuth   = true;                                   //Enable SMTP authentication
            $mail->Username   = 'rcairam@est.unap.edu.pe';                     //SMTP username
            $mail->Password   = 'sqxkcbflwzosudbe';                             //SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
            $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

            //Recipients
            $mail->setFrom('rcairam@est.unap.edu.pe', 'Servidor Local');
            $mail->addAddress($data['email']);     //Add a recipient
            if(!empty($data['emailCopia'])){
                $mail->addBCC($data['emailCopia']);
            }
